...
def generate_text(prompt, model="llama3-8b-8192", max_tokens=2000, temperature=0.7, max_retries=3, retry_delay=5):
    retries = 0
        ...
        try:
            chat_completion = client.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a world-class policy analyst. Analyze the given information and generate a well-structured policy brief. Include URL citations for any facts, statistics or quotes used from the search results using the placeholder <URL_REFERENCES>."
                    },
                    {
                        "role": "user",
                        "content": prompt
            ...
            # Add commas between list elements if the response is a list
...
def generate_subtopic_brief(subtopic, search_data):
    ...
    for i in range(3):
        ...
        try:
            ...
            print(f"Error parsing additional queries for subtopic: {subtopic}. Skipping this subtopic.")
            return "", [], total_tokens
    ...
    print(f"Final queries string: {final_queries}")
    final_queries_str = final_queries.split('[')[1].strip().rstrip(',')

    json_match = re.search(r'\[.*\]', final_queries_str)
    if json_match:
        final_queries_str = json_match.group()
    else:
        print(f"No valid JSON array found for subtopic: {subtopic}. Skipping this subtopic.")
        return "", [], total_tokens

    final_queries = json.loads(final_queries_str)
...
if subtopic_briefs:
    ...
    print("No subtopic briefs generated. Skipping comprehensive policy brief generation.")
    comprehensive_brief = ""
